...
ack扫是确定是否有防火墙,因为无论端口开没开都返回rst，有防火墙返回icmp

syn flood:
udp flood:  无连接，速度快，适合伪造源ip
ack flood:  建立tcp之后发送ack，攻击力不强，因为只有检查，回应rst，且发送小包，经常混合其他攻击
dns flood:  伪造大量用户对dns服务器发起查询服务，服务器会先找缓存，没有再询问上级dns。
http flood: http包结束的标志\r\n\r\n，故意不结束，占用资源
dos攻击防御：
udp -> 指纹识别，工具发送报文带有默认的内容,比如大量重复的内容
syn -> 1.特征匹配 
       2.syn cookie: 先不建立连接, 接收syn后回应syn+ack其中序列号缓存。当对方返回ack时，采用cookie校验，完成校验则建立连接
       商业防护方法
       1.溯源: 向发起syn链接的地址探测,其ip是否真实存在
         safereset算法: 特意构造错误的syn包，等待对方发出rst，接收到rst后建立连接。
       3.syn重传算法: 第一个syn记录并丢弃，第二个建立连接
       4.综合防护算法
...
搜集域名资产，多级域名，IP地址
端口扫描
对端口指纹识别, 数据库端口，web使用组件还是自研
通过网络进行搜索补足
对搜集的资产进行验证，首先是数据库mysql,oracle等，然后系统公共的ftp,smb等，web组件fastjson，strusts2等
